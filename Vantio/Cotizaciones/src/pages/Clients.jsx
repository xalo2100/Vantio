...
const Clients = () => {
    ...
    const handleBulkDelete = async () => {
        ...
        if (!window.confirm(`¿Estás seguro de eliminar ${selectedClients.length} clientes seleccionados? Esta acción no se puede deshacer.`)) return;
        ...
        try {
            ...
            alert('Error al eliminar clientes seleccionados');
        }
    };
    ...
    return (
        <div className="space-y-6">
                ...
                <div className="flex items-center gap-3">
                    {selectedClients.length > 0 && (
                        <button
                            ...
                            className="flex items-center gap-2 bg-red-100 hover:bg-red-200 text-red-700 px-4 py-2.5 rounded-xl font-bold transition-all border border-red-200 animate-in fade-in"
                        >
                            <Trash2 size={20} />
                            <span>Eliminar ({selectedClients.length})</span>
                        </button>
                    )}
                    <button
            ...
            {/* toolbar */}
            <div className="bg-white p-4 rounded-2xl shadow-sm border border-stone-200 flex flex-col md:flex-row gap-4 items-center">
                <div className="flex items-center gap-3 border-r border-stone-200 pr-4 mr-2">
                    ...
                    />
                    <span className="text-sm font-bold text-stone-500">Seleccionar Todo</span>
                </div>
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {clients.map(client => (
                        <div
                            key={client.id}
                            className={`bg-white rounded-2xl p-6 shadow-sm border transition-all group relative ${selectedClients.includes(client.id) ? 'border-orange-500 bg-orange-50' : 'border-stone-200 hover:shadow-md'}`}
                        >
                            <div className="absolute top-4 left-4 z-10">
                                <input
                                    ...
                                    className="w-5 h-5 rounded border-stone-300 text-orange-500 focus:ring-orange-500 cursor-pointer shadow-sm"
                                />
                            </div>

                            <div className="flex justify-end items-start mb-4 pl-10">
