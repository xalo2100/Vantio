  ...
  useEffect(() => {
    if (!loading && !generating && user) {
      ...
      if (!lastGen || (now - parseInt(lastGen)) > oneDay) {
        // Lottery system: only 30% of eligible users trigger auto-generation 
        // to stay within free API limits as requested
        if (Math.random() < 0.3) {
      ...
      {loading ? (
        ...
        <div className="glass-panel p-12 rounded-xl text-center">
          ...
          <h3 className="text-2xl font-bold text-petrol-800 mb-2">
            {filter === 'all' ? 'No hay insights todavía' : 'No hay insights con este filtro'}
          </h3>
          <p className="text-gray-600 mb-6">
            Genera tus primeros insights personalizados con IA
          </p>
          {filter === 'all' && (
            <button
              onClick={handleGenerateWeekly}
            ...
            >
              <Sparkles size={18} />
              {generating ? 'Generando...' : 'Generar Insights Ahora'}
            </button>
          )}
        </div>
      ) : (
