...
const Dashboard = () => {
    ...
    const fetchInsights = async () => {
        try {
            ...
            const { insights } = await insightsService.getUserInsights(3, false);
            setKaizenInsights(insights || []);

            // Auto-generate logic (Quota-friendly)
            if (insights?.length === 0 && user) {
                ...
                if (!lastGen || (now - parseInt(lastGen)) > oneDay) {
                    if (Math.random() < 0.3) {
                        console.log("✨ Dashboard: Triggering automatic Kaizen...");
        ...
        <div className="space-y-8">
            ...
            <div className="space-y-4">
                ...
                {loadingInsights ? (
                    ...
                    <div className="glass-panel p-8 text-center rounded-xl border-2 border-dashed border-gray-200">
                        <Sparkles className="mx-auto text-purple-300 mb-4" size={48} />
                        <h4 className="text-lg font-semibold text-gray-800 mb-2">No hay insights recientes</h4>
                        <p className="text-gray-600 mb-6 max-w-md mx-auto">
                            Kaizen analiza tus ventas y cotizaciones para generarte recomendaciones personalizadas.
                        </p>
                        <button
                            onClick={() => navigate('/kaizen')}
